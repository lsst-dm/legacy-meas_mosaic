...
class StackTask(Task):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.warper = afwMath.Warper.fromConfig(config.warper)
        self.stats = afwMath.StatisticsControl()
    ...
    def run(self, butler, stackId, dataRefList):
        ...
        for dataRef in dataRefList:
            ...
            del exp
            dataRef.put(warp, 'warp', patch=stackId)
            del warp

        stack = self.stack(butler, stackId, dataRefList)
    ...
    def skycell(self, butler, stackId):
        ...
        coord = afwCoord.Coord(22 * afwGeom.arcminutes, -36 * afwGeom.arcminutes)
        point = afwGeom.Point2D(extent / 2)
    ...
    def select(self, dataRefList, skyCell):
        ...
        center = skyCell.wcs.pixelToSky(afwGeom.Point2D(skyCell.bbox.getDimensions() / 2))
        ...
        for dataRef in dataRefList:
            ...
            middle = afwGeom.Point2D(md['NAXIS1'], md['NAXIS2'])
    ...
    def matchPsf(self, exposure):
        """Match PSF of warped exposure"""
        raise NotImplementedError()
    ...
    def stack(self, butler, stackId, dataRefList, skycell):
        ...
        imageList = afwImage.vectorMaskedImageF(len(expIdList))
        for start in range(0, dim.getY(), self.config.rows):
            ...
            for i, dataRef in enumerate(dataRefList):
                exposure = dataRef.get("warp_sub", bbox=box)
