...
class PhotometricSolutionTask(PhotoCalTask):
    ...
    def run(self, matchLists, filterName, wcsList, butler):
        ...
        mlVisit = dict()
        for ccdId in list(matchLists.keys()):
            if matchLists[ccdId] == None:
                continue
            visit, ccd = self.decodeCcdExposureId(ccdId)
            if not visit in list(mlVisit.keys()):
                mlVisit[visit] = list()
            ...
            for m in matches:
                if m.first != None and m.second != None:
                    match = measMosaic.SourceMatch(measMosaic.Source(
                        m.first, wcsList[ccdId]), measMosaic.Source(m.second))
                    match.second.setExp(visit)
                    match.second.setChip(ccd)
                    mlVisit[visit].append(match)

        matchList = measMosaic.SourceMatchGroup()
        for visit in list(mlVisit.keys()):
        ...
        for ccdId in list(matchLists.keys()):
            if matchLists[ccdId] == None:
                continue
            visit, ccd = self.decodeCcdExposureId(ccdId)
            if not ccd in list(ccdSet.keys()):
        ...
        for ccdId in list(wcsList.keys()):
            visit, ccd = self.decodeCcdExposureId(ccdId)
            if not visit in list(wcsDic.keys()) and wcsList[ccdId] != None:
                wcs = wcsList[ccdId]
                ccdDev = ccdSet[ccd]
    ...
    def writeFcr(self, butler, ccdIdList, ccdSet, filterName,
        ...
        for ccdId in ccdIdList:
            ...
            if not ichip in list(ccdSet.keys()):
                continue
            x0 = 0.0
            y0 = 0.0
